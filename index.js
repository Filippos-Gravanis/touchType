...
input.addEventListener("keydown", function (keyPress) {
  setTimeout(() => {
    if (
      ...
      input.value[1] != "" &&
      typedWordsCount < wordList.length
    ) {
      ...
      if (typedWordsCount < wordList.length) {
        displayedWordsList[typedWordsCount].text.style.backgroundColor =
          "#CECCCC";
        console.log(typedWordsCount, wordList.length);
      }
      if (typedWordsCount > 0) {
        displayedWordsList[typedWordsCount - 1].text.style.backgroundColor =
          "white";
      }
      if (typedWordsCount % 10 == 0) {
        ...
        if (wordList.length - typedWordsCount >= 10) {
          for (i = 0; i < 10; i++) {
            displayWordsContainer.append(
              displayedWordsList[typedWordsCount + i].text
            );
          }
        } else {
    ...
    if (
      ...
      if (
        input.value !=
        displayedWordsList[typedWordsCount].text.innerText.substr(
          0,
          input.value.length
        )
      ) {
